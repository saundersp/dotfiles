			...
			-a|a|activate|--activate)
				if [ -f "$VENV_PATH"/Scripts/activate ]; then . "$VENV_PATH"/Scripts/activate
					...
					if [ -f requirements.txt ]; then
						...
						read -r -p 'Would you like to install a Python virtual environment in the current folder ? (Y/N): ' ANS && [[ "$ANS" == '[yY]' || "$ANS" == '[yY][eE][sS]' ]] || return 1
						py c
					else
						echo 'No Python virtual environment not detected'
						return 1
					fi
				fi
			;;
